{ //codice modificato da Gigi e ritoccato da me sulla parte della salvaguardia ai bordi
#include <iostream>
#include <sstream>
#include <memory>
#include <cmath>
...
#include "TTree.h"
  ...
  for (k = 0; k < n_chip; k++) // ciclo sui chip per creare istogrammi, due per ogni chip per analizzare il common mode di ognuno
  {
   ...
   {  //if (i == 79679) || i == 43404 || i == 60212) //scarto evento/i problematico/i
    ...
    for (k = 0; k < n_chip; k++)
    {
      ...
      if (n_strip_common[k] > 0)
      {
        if (n_strip_common[k] < (n_chip_chan / 2))
        {
          ...
          good_event[i] = 0; //flag per indicare che l'evento non è buono
        }
        else
        {
          common[k] = common[k] / n_strip_common[k];
    ...
    if (good_event > 0)
    {
      if (common_tot_rms < 6.)
      {
        ...
        for (k = 0; k < max_cluster_width; k++) //azzero il segnale delle strip
        {
          strip_signal[k] = 0.;
        }
...
// look for relative maximum
             ...
             if (signal[j_index] > cluster_low_threshold)
             {
              ...
              { 
                ...
                {//allora...
                  local_max[i][i_local_max][3] = float(j_index); //...memorizzo  la posizione del massimo..
                  local_max[i][i_local_max][2] = signal[j_index]; //..e memorizzo il valore del nuovo massimo locale.
             ...
             else //altrimenti (non ho strip che mi superano soglia) non inizio il cluster oppure lo chiudo
             {
              if (in_cluster > 0) // se ho una o piu strip nel cluster (cioè ho un cluster aperto) 
              {
                ...
                if (local_max[i][i_local_max][0] > 2) // se lo start > 3
                {
                  Int_t lower_cluster =  int(local_max[i][i_local_max][0])-3; // prendo lo start meno 3
                  if (local_max[i][i_local_max][1] < n_strip_sensor-2) // se l'end < 637
                  { Int_t upper_cluster = int(local_max[i][i_local_max][1])+3; //prendo l' end piu 3
